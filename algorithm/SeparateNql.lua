...
local nql = torch.class('dqn.SeparateNql')
...
function nql:__init(args)
    ...
    if args.network:sub(-3) == '.t7' then
        ...
        if self.best == 1 and exp.best_model then
            ...
            self.mem_network = exp.mem_best_model[1]
        elseif exp.model then
            self.network = exp.model
            self.mem_network = exp.mem_model
        else
            self.network = exp
            self.mem_network = mem_exp
        end
        ...
        self.mem_network = self:create_mem_q()
    end
...
function nql:getQUpdate(args)
    ...
    local target_q_net
    if self.target_q then
        mem_target_q_net = self.mem_target_network
        target_q_net = self.target_network
    else
        mem_target_q_net = self.mem_network
        target_q_net = self.network
    end

    -- Compute max_a Q(s_2, a).
    mem_target_q_net:evaluate()
    target_q_net:evaluate()
    local output2 = {mem_target_q_net:forward(s2), target_q_net:forward(s2)}
    ...
    local output = {self.mem_network:forward(s), self.network:forward(s)}
...
function nql:qLearnMinibatch()
    ...
    -- get new gradient
    self.mem_network:backward(s, targets.mem)
    self.network:backward(s, targets.beh)
    ...
    -- Memory
    -- clip gradients
    local mem_grad_norm = self.mem_dw:norm()
    if self.clip_grad > 0 and mem_grad_norm > self.clip_grad then
        self.mem_dw:mul(self.clip_grad / mem_grad_norm)
    end
...
function nql:greedy(state)
    ...
    local output = {self.mem_network:forward(state), self.network:forward(state)}
...
function nql:report()
    print(get_weight_norms(self.mem_network.net))
    print(get_grad_norms(self.mem_network.net))
    print("Grad Norm(mem): " .. tostring(self.mem_dw:norm()))

    print(get_weight_norms(self.network.net))
    print(get_grad_norms(self.network.net))
    print("Grad Norm(beh): " .. tostring(self.dw:norm()))
end

function nql:create_mem_q()
    local new_args = {}
    new_args.name               = self.name
    new_args.hist_len           = self.hist_len or 10
    new_args.n_actions          = new_args.hist_len - 1
