...
function nql:getQUpdate(args)
    ...
    local output2 = target_q_net:forward(s2, times2, recall2, noise2)
    ...
    local output = self.network:forward(s, times, recall, noise)
...
function nql:perceive(reward, rawstate, terminal, testing, testing_ep)
    ...
    local state = {frames=curState, time=times, recall=recalls, noise=noise}
...
function nql:greedy(state)
    ...
    local output = self.network:forward(state, times, recall, noise)
    ...
    return {mem_action, beh_action, atten, reten, stren, sigma, comps}
end
