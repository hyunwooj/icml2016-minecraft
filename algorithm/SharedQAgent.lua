...
local agent = torch.class('dqn.SharedQAgent')
...
function agent:learn()
    ...
    local output2 = self.target_network:forward(s2)
    local mem_q2_max = output2[1]:float():clone():max(2)
    local beh_q2_max = output2[2]:float():clone():max(2)
    ...
    local mem_q_all = output[1]:float():clone()
    local beh_q_all = output[2]:float():clone()
    ...
    self:update_parameter(s, mem_targets, beh_targets)
end
